...
function SidebarButton({ name, icon, link, borderBottom, radiusTr, radiusBr }) {
	return (
		<Tooltip label={name} hasArrow placement="right" rounded="5px" bg="#0284c7" w="125px" className="flex items-center justify-center text-center">
			<NavLink to={link} className={`flex justify-center items-center text-white hover:bg-sky-700 h-16 xl:h-20 ${borderBottom} ${radiusTr} ${radiusBr} border-slate-700`}>
				{icon}
			</NavLink>
		</Tooltip>
	);
...
export default function Sidebar() {
	...
	return <div className="flex flex-col w-20 rounded-tr-xl rounded-br-xl blue-swbt me-7">{renderViews}</div>;
}
